...
experiment Batch type: batch repeat: 1 keep_seed: true until: (food_gathered = food_placed) or (time > 400) {
  
   parameter 'Number:' var: ants_number min: 10 max: 300 unit: 'ants' step: 5;
   parameter 'Evaporation:' var: evaporation_per_cycle min: 5.0 max: 240.0 unit: 'rate every cycle (1.0 means 100%)' step: 0.3;
   parameter 'Diffusion:' var: diffusion_rate min: 0.1 max: 1.0 unit: 'rate every cycle (1.0 means 100%)' step: 0.3;
    ...
    ask simulations {
    save [int(self),ants_number,evaporation_per_cycle,diffusion_rate,food_gathered] 
          to: "ant_results.csv" type: "csv" rewrite: (int(self) = 0) ? true : false header: true;
    }       
}
}
